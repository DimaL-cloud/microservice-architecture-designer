...
- **Generation state lives only in memory.** On restart, `GenerationRecoveryListener` marks any project stuck in `GENERATING` as `FAILED` (clearing a leftover summary placeholder but keeping a real summary). Recover via `POST /api/projects/{id}/restart-generation`, which re-runs from the persisted brief and **reuses an already-persisted blueprint and summary** (so a bad blueprint survives restarts, and a good summary isn't re-billed or clobbered).
