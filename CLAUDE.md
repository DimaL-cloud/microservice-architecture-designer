...
2. **Save & Generate** (`ProjectGenerationService.saveAndGenerate`, `POST /api/projects`) — generates a best-effort summary, commits the `Project` row as `GENERATING` **in its own transaction**, then fires the async orchestrator and returns `201` immediately. The frontend polls `GET /api/projects/{id}`.
3. **Orchestration** (`ArtifactGenerationOrchestrator.generate`, `@Async("artifactGenerationExecutor")`):
   - **Blueprint** (`BlueprintService`) — the **only** structured-output (`.entity`) LLM call. Produces `ArchitectureBlueprint` (system overview, containers, relationships, decisions, key flows). Decisions are clamped to `max-adrs` (8) and flows to `max-flows` (6) so fan-out can't grow unboundedly.
...
- **The blueprint is the only structured-output call.** Everything else is raw `.content()` text stored/validated directly.
...
- **Generation state lives only in memory.** On restart, `GenerationRecoveryListener` marks any project stuck in `GENERATING` as `FAILED`. Recover via `POST /api/projects/{id}/restart-generation`, which re-runs from the persisted brief and **reuses an already-persisted blueprint** (so a bad blueprint survives restarts).
